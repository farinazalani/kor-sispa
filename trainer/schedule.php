...
$trainer_courses = $conn->query("SELECT 
    c.CourseID,
    c.CourseName,
    c.Description,
...
// Get all schedules for trainer's courses
$schedules = [];
if (count($trainer_courses) > 0) {
    $course_ids = array_column($trainer_courses, 'CourseID');
...
foreach ($schedules as $schedule) {
    ...
    $calendar_events[] = [
        ...
        'enrolled' => $schedule['EnrolledCount']
    ];
}
...
// Calculate total enrolled students across all trainer's courses
$total_enrolled = 0;
foreach ($trainer_courses as $course) {
    $enroll_count = $conn->query("SELECT COUNT(*) as cnt FROM enrollments WHERE CourseID = {$course['CourseID']} AND Status = 'Enrolled'")->fetch_assoc();
    $total_enrolled += $enroll_count['cnt'];
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>My Schedule - Trainer</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-success { background-color: #00b85c; color: white; }
        ...
        .enrolled-badge { background: var(--success); padding: 2px 8px; border-radius: 12px; font-size: 0.65rem; }
        ...
        .modal-content { background: var(--bg-card); padding: 0; border-radius: 12px; width: 90%; max-width: 480px; position: relative; }
        ...
        .close { font-size: 28px; cursor: pointer; color: var(--text-secondary); line-height: 1; }
        .close:hover { color: var(--danger); }
        .form-group { margin-bottom: 15px; }
        .form-group label { display: block; margin-bottom: 5px; font-weight: 500; }
        .form-group input, .form-group textarea, .form-group select { width: 100%; padding: 8px 12px; border: 1px solid var(--border); border-radius: 4px; background: var(--calendar-day-bg); color: var(--text-main); }
        .form-group input[readonly] { background: var(--table-header); cursor: not-allowed; }
        
        .duration-selector { display: flex; align-items: center; gap: 8px; }
        .duration-btn-sm { width: 28px; height: 28px; font-size: 16px; font-weight: bold; border: 1px solid var(--border); background: var(--table-header); border-radius: 4px; cursor: pointer; color: var(--text-main); }
        .duration-value-sm { font-size: 16px; font-weight: bold; min-width: 50px; text-align: center; padding: 4px 8px; border: 1px solid var(--border); border-radius: 4px; background: var(--calendar-day-bg); }
        ...
        .class-cell .location { font-size: 0.55rem; opacity: 0.9; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .class-cell .enrolled { font-size: 0.55rem; opacity: 0.8; margin-top: 2px; }
        ...
        .add-schedule-btn { margin-bottom: 15px; }

        @media print { .sidebar, .top-bar, .week-nav, .stats-grid, .print-btn, .btn-logout, .view-tabs, .add-schedule-btn { display: none !important; } .main-content { margin-left: 0 !important; padding: 0 !important; } }
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <?php if (count($trainer_courses) == 0): ?>
                <div class="alert alert-warning">⚠️ You haven't been assigned to any courses yet. Contact administrator.</div>
            <?php endif; ?>

            <!-- Stats Cards -->
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-number"><?php echo count($trainer_courses); ?></div><div class="stat-label">My Courses</div></div>
                ...
                <div class="stat-card"><div class="stat-number"><?php echo $total_enrolled; ?></div><div class="stat-label">Total Students</div></div>
            </div>
            ...
            <div id="view-list" class="view-section">
                <div class="table-container">
                    <div class="add-schedule-btn">
                        <button class="btn-sm btn-primary" onclick="openAddScheduleForm()">+ Add New Schedule</button>
                    </div>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><span class="enrolled-badge">👥 <?php echo $schedule['EnrolledCount']; ?> enrolled</span></td>
                                                <td>
                                                    <button class="btn-sm btn-warning" onclick="editSchedule(<?php echo $schedule['ScheduleID']; ?>)">Edit</button>
                                                    <button class="btn-sm btn-danger" onclick="deleteSchedule(<?php echo $schedule['ScheduleID']; ?>)">Del</button>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
            ...
            <div id="view-timetable" class="view-section" style="display:none;">
                <?php if(count($schedules) > 0): ?>
                    ...
                    <div style="margin-top: 15px; padding: 10px; background: var(--bg-card); border-radius: 6px; font-size: 0.65rem;">
                        <span>📖 Legend: 🎓 Course | 📍 Location | 👥 Enrolled | 💡 Click for details</span>
                    </div>
                <?php else: ?>
                    <div class="empty-state"><p>📭 No schedules added yet.</p></div>
                <?php endif; ?>
            </div>
        ...
        <div class="modal-content">
            ...
            <form id="scheduleForm">
                <div class="modal-body">
                    ...
                    <input type="hidden" name="course_id" id="schedule_course_id">
                    <input type="hidden" name="duration_hours" id="durationHours" value="1">
                    
                    <div class="form-group">
                        <label>Course *</label>
                        <select name="course_id_display" id="course_id_display" required>
                    ...
                    <div class="form-group">
                        ...
                        <div class="duration-selector">
                            <button type="button" class="duration-btn-sm" id="durationMinus">-</button>
                            <span class="duration-value-sm" id="durationValue">1</span>
                            <button type="button" class="duration-btn-sm" id="durationPlus">+</button>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="location" id="location" placeholder="e.g. Bilik A-101">
                    </div>
                    ...
                    <div id="timeConflictWarning" style="display: none; background: #fee2e2; color: #991b1b; padding: 8px; border-radius: 4px; margin-top: 10px; font-size: 0.8rem;">
                        ⚠️ This time slot conflicts with an existing schedule.
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn-sm btn-secondary" onclick="closeScheduleModal()">Cancel</button>
                    <button type="submit" class="btn-sm btn-primary" id="saveScheduleBtn">Save Schedule</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="eventModal" class="modal" onclick="closeModal()">
        <div class="modal-content" onclick="event.stopPropagation()">
            ...
            <p><strong>👥 Enrolled:</strong> <span id="modalEnrolled"></span> students</p>
    ...
    <script>
        ...
        const trainerCourses = <?php echo json_encode($trainer_courses); ?>;
        ...
        function buildTimetable() {
            ...
            for(let slot of timeSlots) {
                ...
                for(let i=0; i<days.length; i++) {
                    ...
                    if(schedule) {
                        bodyHtml += `<td><div class="class-cell" onclick="openModal(${schedule.ScheduleID})"><div class="course-name">${escapeHtml(schedule.CourseName.substring(0,16))}</div><div class="location">📍${escapeHtml((schedule.Location||'TBA').substring(0,10))}</div><div class="enrolled">👥 ${schedule.EnrolledCount}</div></div></td>`;
        ...
        function updateEndTime() {
            ...
            if (startTime && currentDuration) {
                const endTime = calculateEndTime(startTime, currentDuration);
        ...
        function setupDurationButtons() {
            ...
            minusBtn.addEventListener('click', function() {
                let newVal = currentDuration - 1;
                if (newVal >= 1) {
                    currentDuration = newVal;
        ...
        async function loadOccupiedStartTimes(date, courseId, scheduleId) {
            if (!date) return;
            try {
                const response = await fetch('../actions/course-schedule-action.php?action=get_occupied_start_times&date=' + encodeURIComponent(date) + '&exclude_course_id=' + courseId + '&exclude_schedule_id=' + (scheduleId || ''));
                const data = await response.json();
                if (data.success) {
                    occupiedStartTimes = data.occupiedStartTimes || [];
                    updateTimeDropdown();
                }
            } catch (error) {
        ...
        function updateTimeDropdown() {
            ...
            for (let i = 0; i < timeSelect.options.length; i++) {
                ...
                const isOccupied = occupiedStartTimes.includes(option.value);
                if (isOccupied) {
                    option.disabled = true;
                    option.style.color = '#9ca3af';
                    option.title = 'Already taken by another course on this date';
                }
            }
        ...
        async function checkConflict() {
            const courseId = document.getElementById('course_id_display').value;
            ...
            if (!courseId || !scheduleDate || !startTime || !currentDuration) {
            ...
            if (!endTime) {
                ...
                warningDiv.innerHTML = '⚠️ End time exceeds 23:00';
                saveBtn.disabled = true;
                return;
            }
            
            try {
                const url = '../actions/course-schedule-action.php?action=check_conflict&course_id=' + courseId + '&schedule_date=' + scheduleDate + '&start_time=' + startTime + '&end_time=' + endTime + '&exclude_id=' + (scheduleId || '');
        ...
        async function handleDateChange() {
            const courseId = document.getElementById('course_id_display').value;
            const scheduleId = document.getElementById('schedule_id').value;
            await loadOccupiedStartTimes(this.value, courseId, scheduleId);
            updateEndTime();
            await checkConflict();
        }
        ...
        function openAddScheduleForm() {
            ...
            currentDuration = 1;
            document.getElementById('durationValue').textContent = '1';
            ...
            const courseSelect = document.getElementById('course_id_display');
            dateInput.onchange = handleDateChange;
            startSelect.onchange = function() { updateEndTime(); checkConflict(); };
            courseSelect.onchange = async function() { 
                document.getElementById('schedule_course_id').value = this.value;
                const date = document.getElementById('schedule_date').value;
                if (date) await loadOccupiedStartTimes(date, this.value, '');
                checkConflict();
            };
        }
        ...
        function editSchedule(scheduleId) {
            ...
            document.getElementById('schedule_course_id').value = schedule.CourseID;
            document.getElementById('course_id_display').value = schedule.CourseID;
            document.getElementById('schedule_date').value = schedule.ScheduleDate;
            document.getElementById('start_time').value = schedule.StartTime;
            document.getElementById('location').value = schedule.Location || '';
            document.getElementById('schedule_status').value = schedule.ScheduleStatus;
            ...
            document.getElementById('durationHours').value = currentDuration;
            document.getElementById('end_time').value = schedule.EndTime.substring(0,5);
            ...
            courseSelect.onchange = async function() { 
                document.getElementById('schedule_course_id').value = this.value;
                const date = document.getElementById('schedule_date').value;
                if (date) await loadOccupiedStartTimes(date, this.value, scheduleId);
                checkConflict();
            };
            
            loadOccupiedStartTimes(schedule.ScheduleDate, schedule.CourseID, scheduleId);
            document.getElementById('scheduleModal').style.display = 'flex';
        }

        function deleteSchedule(scheduleId) {
            if (confirm('Delete this schedule? This action cannot be undone.')) {
                fetch('../actions/course-schedule-action.php', {
                    method: 'POST',
        ...
        document.getElementById('scheduleForm').addEventListener('submit', async function(e) {
            ...
            if (!startTime || !courseId) {
                alert('Please select course and start time');
                return;
            }
            
            const endTime = calculateEndTime(startTime, currentDuration);
            if (!endTime) {
                alert('End time exceeds 23:00');
                return;
            }
            ...
            const data = await response.json();
            if (data.hasConflict) {
                alert('Time conflict! This slot is already taken.');
                return;
            }
            
            const action = scheduleId ? 'update_schedule' : 'add_schedule';
            const formData = new FormData(this);
            formData.append('action', action);
            formData.append('end_time', endTime);
            formData.append('duration_hours', currentDuration);
            ...
            .catch(error => { alert('Failed to save schedule.'); });
        });
        ...
        function openModal(id) {
            ...
            if(!e) return;
            document.getElementById('modalCourse').innerHTML = e.title;
            document.getElementById('modalTime').innerHTML = e.full_time;
            document.getElementById('modalLocation').innerHTML = e.location;
            document.getElementById('modalDate').innerHTML = e.day + ', ' + e.date;
            document.getElementById('modalEnrolled').innerHTML = e.enrolled;
