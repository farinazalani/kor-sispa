...
// Get the minimum and maximum dates that have data for this cadet
$date_range_query = "
    SELECT MIN(DATE(TestDate)) as min_date, MAX(DATE(TestDate)) as max_date
    FROM physical_performance
    WHERE UserID = ?
";
...
// Updated query to match the single-row structure of the physical_performance table with date filters
$query = "
    SELECT pp.*, rec.Fullname AS RecordedByName
...
$query .= " ORDER BY pp.TestDate DESC";
...
<head>
    ...
    <title>My Physical Tests - Kor SISPA</title>
    <link rel="stylesheet" href="../assets/css/style.css">
    <style>
        /* Score cells */
        .score-cell { text-align: center; font-weight: 600; }
        
        .remark-text { font-style: italic; color: #666; font-size: 0.9em; }

        /* Print-friendly */
        @media print {
            .sidebar, .top-bar, .btn-logout, .btn-download { display: none !important; }
        ...
        /* Report header */
        .report-header { text-align:center; margin-bottom:20px; display:none; }
        .report-header img { height:80px; margin-bottom:10px; }
        
        .btn-download {
            display:inline-block; padding:10px 20px; background-color:#667eea; color:#fff;
            text-decoration:none; border-radius:6px; margin-bottom:15px; font-weight: bold;
        }
        .btn-download:hover { background-color:#5563c1; }
...
<div class="dashboard-container">
    ...
    <div class="main-content">
        ...
        <div class="report-header">
            <img src="../assets/images/logo_korsispa.png" alt="KOR SISPA Logo">
            <h1>Kor SISPA Physical Fitness Report</h1>
            <p>Cadet: <?php echo htmlspecialchars($_SESSION['fullname']); ?></p>
            <p>Generated on: <?php echo date('d M Y, H:i A'); ?></p>
        </div>

        <div class="table-container">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                <h3>Physical Fitness Records</h3>
            </div>
            ...
            <form method="GET" action="" class="filter-container" id="filterForm">
                ...
                <?php if ($date_start || $date_end): ?>
                    <a href="?date_start=&date_end=" class="btn-reset">Reset</a>
                <?php endif; ?>
                <?php if ($date_start || $date_end): ?>
            ...
            <table>
                ...
                <?php if (count($result) > 0): ?>
                    <?php foreach ($result as $row): ?>
                        <tr>
                            <td><strong><?php echo date('d M Y', strtotime($row['TestDate'])); ?></strong></td>
                            <td class="score-cell"><?php echo $row['PushUps']; ?></td>
                            <td class="score-cell"><?php echo $row['SitUps']; ?></td>
                            <td class="score-cell"><?php echo $row['PullUps']; ?></td>
                            <td class="score-cell">
                                <?php echo number_format($row['Running24km'], 2); ?> min
                            </td>
                            <td class="remark-text">
                                <?php echo !empty($row['Remark']) ? htmlspecialchars($row['Remark']) : '<span style="color:#ccc;">No remarks</span>'; ?>
                            </td>
                            <td>
                                <small><?php echo htmlspecialchars($row['RecordedByName']); ?></small>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="7" style="text-align:center; padding: 30px; color: #666;">
                            <?php if ($date_start || $date_end): ?>
                                No physical test records found in the selected date range. Keep training!
                            <?php elseif (empty($min_date)): ?>
                                No physical test records found yet. Keep training!
                            <?php else: ?>
                                No physical test records found. Keep training!
                            <?php endif; ?>
                        </td>
                    </tr>
                <?php endif; ?>
