...
if ($show_expired !== 'yes') {
    $where_clauses[] = "(a.ExpiryDate IS NULL OR a.ExpiryDate >= CURDATE())";
}
...
$expired_count = $conn->query("SELECT COUNT(*) as cnt FROM announcements WHERE ExpiryDate IS NOT NULL AND ExpiryDate < CURDATE()")->fetch_assoc();
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <div class="expired-notice">
                ...
                <a href="?show_expired=yes&<?php echo http_build_query(array_filter(['priority' => $priority_filter, 'year' => $selected_year, 'month' => $selected_month, 'search' => $search])); ?>" class="show-expired-btn">
            ...
            <div class="expired-notice" style="background: #e2e3e5; border-left-color: #6c757d; color: #383d41;">
                ...
                <a href="?<?php echo http_build_query(array_filter(['priority' => $priority_filter, 'year' => $selected_year, 'month' => $selected_month, 'search' => $search])); ?>" class="show-expired-btn" style="background: #28a745;">
            ...
            <?php if ($has_urgent && $show_expired !== 'yes'): ?>
            <div class="urgent-warning">
                <strong><i class="fas fa-exclamation-triangle"></i> ATTENTION!</strong> There are URGENT announcements that require immediate action. Please read them now.
            </div>
            <?php endif; ?>
                        ...
                        <div class="card" style="<?php echo $announcement['Priority'] === 'Urgent' ? 'border-left: 4px solid #dc3545;' : ($announcement['Priority'] === 'Event' ? 'border-left: 4px solid #28a745;' : ''); ?>">
                            ...
                            <small>
                                ...
                                <span style="<?php echo $announcement['Priority'] === 'Urgent' ? 'color: #dc3545; font-weight: bold;' : ($announcement['Priority'] === 'Event' ? 'color: #28a745;' : ''); ?>">
                                    <?php 
                                    if ($announcement['Priority'] == 'Urgent') echo '🔴 Urgent (Expires after 3 days)';
                                    elseif ($announcement['Priority'] == 'Event') echo '🟢 Event (Expires 1 day after event)';
                                    else echo '⚪ Normal (Expires after 15 days)';
                                    ?>
                                </span><br>
            ...
            <form action="../actions/announcement-action.php" method="POST" id="addForm">
                ...
                <div class="form-group">
                    ...
                    <select name="priority" id="priority_select" required>
                        <option value="Normal">⚪ Normal (Expires after 15 days from post date)</option>
                        <option value="Urgent">🔴 Urgent (Expires after 3 days from post date)</option>
