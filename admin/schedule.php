...
// Get all schedules from all courses
$schedules = [];
    ...
    foreach ($time_slots as $slot) {
        if ($start_time >= $slot['start'] && $start_time < date('H:i:s', strtotime($slot['start'] . ' +1 hour'))) {
            ...
            $content = sprintf(
                ...
                htmlspecialchars(substr($schedule['CourseName'], 0, 14)),
                htmlspecialchars($trainer),
                htmlspecialchars(substr($schedule['Location'] ?? 'TBA', 0, 8)),
                $schedule['EnrolledCount']
            );
...
$total_enrolled = 0;
$enroll_result = $conn->query("SELECT COUNT(*) as cnt FROM enrollments WHERE Status = 'Enrolled'");
if ($enroll_result) {
    $total_enrolled = $enroll_result->fetch_assoc()['cnt'];
}
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .tab-btn.active { background: var(--accent-hover); color: white; border-color: var(--accent); }
        ...
        .schedule-badge { display: inline-block; background: var(--accent-hover); color: white; padding: 4px 10px; border-radius: 20px; font-size: 0.7rem; margin: 2px; }
        ...
        .week-nav-btn { background: var(--bg-body); color: white; border: none; padding: 5px 12px; border-radius: 5px; cursor: pointer; font-size: 0.75rem; font-weight: 600; }
        ...
        .day-col { background: var(--table-header); font-weight: bold; text-align: center; width: 110px; }
        ...
        .class-cell .trainer-name { font-size: 0.55rem; opacity: 0.9; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .class-cell .location { font-size: 0.55rem; opacity: 0.8; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
<body>
    ...
    <div class="dashboard-container">
        ...
        <div class="main-content">
            ...
            <?php if (count($schedules) == 0): ?>
                <div class="alert alert-warning">⚠️ No schedules have been created yet.</div>
            <?php endif; ?>

            <!-- Stats Cards -->
            <div class="stats-grid">
                <div class="stat-card"><div class="stat-number"><?php echo $total_courses; ?></div><div class="stat-label">Total Courses</div></div>
                <div class="stat-card"><div class="stat-number"><?php echo $total_schedules; ?></div><div class="stat-label">Total Classes</div></div>
                <div class="stat-card"><div class="stat-number"><?php echo $classes_this_week; ?></div><div class="stat-label">Classes This Week</div></div>
                <div class="stat-card"><div class="stat-number"><?php echo $total_enrolled; ?></div><div class="stat-label">Total Enrollments</div></div>
            </div>
            ...
            <div id="view-list" class="view-section">
                <div class="table-container">
                    <h3>📋 All Class Schedules (All Trainers)</h3>
                    <?php if(count($schedules) > 0): ?>
                        <?php foreach ($schedules_by_course as $course_id => $course_data): ?>
                            <div style="margin-bottom: 25px;">
                                <h4 style="color: var(--accent); margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px solid var(--border);">
                                    ...
                                    <small style="color: var(--text-secondary); font-size: 0.7rem;">(Trainer: <?php echo htmlspecialchars($course_data['trainer']); ?>)</small>
                                </h4>
                                    ...
                                    <tbody>
                                        ...
                                        ?>
                                            <tr>
                                                ...
                                                <td><span class="enrolled-badge">👥 <?php echo $schedule['EnrolledCount']; ?></span></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
            ...
            <div id="view-timetable" class="view-section" style="display:none;">
                <?php if(count($schedules) > 0): ?>
                    ...
                    <div style="margin-top: 15px; padding: 10px; background: var(--bg-card); border-radius: 6px; font-size: 0.65rem;">
                        <span>📖 Legend: 🎓 Course | 👨‍🏫 Trainer | 📍 Location | 👥 Enrolled | 💡 Click for details</span>
                    </div>
                <?php else: ?>
                    <div class="empty-state"><p>📭 No schedules available.</p></div>
                <?php endif; ?>
    ...
    <script>
        ...
        function buildTimetable() {
            ...
            for(let slot of timeSlots) {
                ...
                for(let i=0; i<days.length; i++) {
                    ...
                    if(schedule) {
                        let trainer = (schedule.TrainerName || '').replace(/^(DR|PROF|DATIN|HAJAH|USTAZ)\s+/i, '').substring(0,8);
                        let courseName = schedule.CourseName.substring(0,14);
                        let location = (schedule.Location || 'TBA').substring(0,8);
                        bodyHtml += `<td><div class="class-cell" onclick="openModal(${schedule.ScheduleID})"><div class="course-name">${escapeHtml(courseName)}</div><div class="trainer-name">${escapeHtml(trainer)}</div><div class="location">📍${escapeHtml(location)}</div><div class="enrolled">👥 ${schedule.EnrolledCount}</div></div></td>`;
        ...
        function openModal(id) {
            ...
            if(!e) return;
            document.getElementById('modalCourse').innerText = e.title;
            document.getElementById('modalTrainer').innerText = e.trainer;
            document.getElementById('modalTime').innerText = e.full_time;
            document.getElementById('modalLocation').innerText = e.location;
            document.getElementById('modalDate').innerText = e.day + ', ' + e.date;
            document.getElementById('modalEnrolled').innerText = e.enrolled;
